...
export default function DataTable({ selectedCategory, selectLowStock, headers }) {
    ...
    if (selectedCategory === 'All Category' && selectLowStock !== 'Low Stock') {
        ...
        sortedData = billingData;
    } else {
        sortedData = selectedCategory
            ? billingData.filter((item) => item.category === selectedCategory)
            : billingData;

        if (selectLowStock === 'Low Stock') {
            sortedData = billingData.filter((item) => item.stock < 10);
        }
    }
                            ...
                            <StyledTableRow key={index}>
                                {headers.map((header) => (
                                    <StyledTableCell key={header.key} align='center'>
                                        {header.key === 'billingNumber' && row[header.key]}
                                        {header.key === 'type' && row[header.key]}
                                        {header.key === 'status' && row[header.key]}
                                        {header.key === 'price' && row[header.key]}
                                        {header.key === 'paymentMethod' && row[header.key]}
                                        {header.key === 'invoiceDate' && row[header.key]}
                                    </StyledTableCell>
                                ))}
                            </StyledTableRow>
                        ))}
